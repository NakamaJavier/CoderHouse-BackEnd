...
export const passportError = (strategy) => {
    return async (req, res, next) => {
        passport.authenticate(strategy, (error, user, info) => {
            ...
            if (!user) {
                console.error({error: info.messages ? info.messages : info.toString()});
                return res.status(401).redirect("/static/userlog") //Si me envian info.messages, muestro la respuesta que me enviaron sino muestro el objeto info pasado a string (pueden enviar info.messages = "Usuario no valido" o info = "User no validado")
            }
            req.user = user
            next()
